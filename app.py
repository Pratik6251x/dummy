...
app_code = r'''from flask import Flask, render_template, request, redirect, url_for, session, jsonify, flash
...
def login():
    if request.method == "POST":
        ...
        try:
            ...
            if user is None:
                print(f"LOGIN FAILED: user not found: {username!r}")
            ...
            if not stored_password:
                print(f"LOGIN FAILED: empty password hash for user: {username!r}")
                flash("This account has an invalid password. Please register again.", "error")
                return render_template("login.html")
            ...
            if not password_ok:
                print(f"LOGIN FAILED: wrong password for user: {username!r}")
                flash("Invalid username or password.", "error")
                return render_template("login.html")
            ...
            if last:
                try:
                    ...
                    days_since_login = (today - last_date).days
            ...
            print(f"LOGIN SUCCESS: {username!r}")

            return redirect(url_for("dashboard"))

        except Exception as e:
            print("LOGIN ERROR:", repr(e))
            flash("Something went wrong while logging in. Check the VS Code terminal.", "error")
            return render_template("login.html")
...
def dashboard():
    ...
    challenges = db.execute(
        "SELECT * FROM daily_challenges ORDER BY RANDOM() LIMIT 3"
    ).fetchall()
    ...
    leaderboard = db.execute(
        """
        SELECT username, total_study_time, streak_count
        FROM users
...
def save_session():
    ...
    db.execute(
        ...
        (user_id, duration, subject, start_time)
        VALUES (?, ?, ?, ?)
        """,
...
def set_timer_default():
    ...
    db.execute(
        "UPDATE users SET default_timer = ? WHERE id = ?",
        (mins, session["user_id"]),
    )
    db.commit()
...
def mood():
    ...
    mood_counts = {
        m: 0
        for m in ["Happy", "Focused", "Tired", "Stressed", "Relaxed"]
    }
...
def ai_tips():
    ...
    tips = {
        "Happy": [
            "Channel your energy into hard topics!",
            "Great time for creative projects.",
            "Tackle your most challenging tasks now.",
            "Use this mood to set ambitious goals.",
        ],
        "Focused": [
            "Perfect for deep work sessions.",
            "Try a 90-min flow sprint.",
            "Minimize notifications and go deep.",
            "This is your peak performance state!",
        ],
        "Tired": [
            "Take a 20-min power nap first.",
            "Do light review instead of new material.",
            "Hydrate and take short breaks.",
            "Try 30-min sessions with 10-min breaks.",
        ],
        "Stressed": [
            "Start with the easiest task to build momentum.",
            "Try 5 min of box breathing.",
            "Break big tasks into tiny steps.",
            "Reward yourself after each small win.",
        ],
        "Relaxed": [
            "Good for reading and passive review.",
            "Great time to plan your week.",
            "Try creative brainstorming.",
            "Review flashcards or notes slowly.",
        ],
    }

    mood_tips = tips.get(mood, tips["Focused"])
    selected = random.sample(mood_tips, min(3, len(mood_tips)))
    ...
    return jsonify({
        "tips": selected,
        "mode": study_modes.get(mood, "Focus Mode"),
    })
...
def analytics():
    ...
    subject_data = {
        s["subject"]: round(s["total"] / 60, 1)
        for s in subjects
    }

    mood_trend = db.execute(
        """
        SELECT mood, DATE(timestamp) as day
        FROM mood_logs
        WHERE user_id = ?
        ORDER BY timestamp DESC
        LIMIT 14
...
def leaderboard():
    ...
    board = db.execute(
        """
        SELECT username, avatar, total_study_time, streak_count
        FROM users
        ORDER BY total_study_time DESC
        LIMIT 20
        """
...
def set_goal():
    ...
    if existing:
        db.execute(
            """
            UPDATE weekly_goals
            SET goal_text = ?, target_hours = ?
            WHERE id = ?
            """,
            (
                data.get("goal_text", ""),
                data.get("target_hours", 10),
                existing["id"],
            ),
        )
    else:
        db.execute(
            ...
            (user_id, goal_text, target_hours, current_hours, week_start)
            VALUES (?, ?, ?, 0, ?)
            """,
            (
                uid,
...
def add_reminder():
    ...
    db.execute(
        ...
        (user_id, title, reminder_time, days)
        VALUES (?, ?, ?, ?)
        """,
        (
            uid,
            data.get("title"),
            data.get("time"),
            data.get("days", "Mon,Tue,Wed,Thu,Fri"),
        ),
    )
    db.commit()
...
def chat():
    ...
    try:
        ...
        return jsonify({
            "reply": "I'm having a little trouble connecting right now. Try again in a moment! ☀️"
        }), 200
...
def weekly():
    ...
    for w in range(4):
        ...
        top_subject = db.execute(
            """
            SELECT subject, SUM(duration) as dur
            FROM study_sessions
            WHERE user_id = ?
              AND DATE(start_time) BETWEEN ? AND ?
            GROUP BY subject
...
{% block title %}Login – SunSync{% endblock %}
...
<div class="auth-wrap">
  <div class="auth-card animate-in">
    ...
    <p class="auth-sub">
      Sign in to your SunSync account
    </p>
...
out = Path("/mnt/data/SunSync_login_fix")
out.mkdir(exist_ok=True)

(out / "app.py").write_text(app_code, encoding="utf-8")
(out / "login.html").write_text(login_html, encoding="utf-8")

zip_path = Path("/mnt/data/SunSync_login_fix.zip")
with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as z:
    z.write(out / "app.py", "app.py")
    z.write(out / "login.html", "templates/login.html")
